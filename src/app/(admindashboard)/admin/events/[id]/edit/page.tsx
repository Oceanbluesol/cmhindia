// app/(admindashboard)/admin/events/[id]/edit/page.tsx
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-6 flex items-center justify-between">
        ...
        <a href="/admin/events" className="text-sm text-gray-600 hover:text-indigo-600">
          ← Back
        </a>
      </div>

      {/* UPDATE FORM (no encType/method; server actions handle it) */}
      <form action={updateEventAdmin} className="space-y-6 rounded-xl border bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              If you upload a new file, it will replace the current poster.
            </p>
          </div>
            ...
            {e.poster_url ? (
              ...
              <a href={e.poster_url} target="_blank" className="group block w-full">
                <img
                  src={e.poster_url}
                  alt={e.name}
                  className="h-32 w-full rounded-lg object-cover ring-1 ring-gray-200 group-hover:opacity-90"
                />
        ...
        {/* Save button ONLY (no nested form here) */}
        <div className="flex items-center justify-between">
          <a href={`/admin/events/${e.id}`} className="text-sm text-gray-600 hover:text-indigo-600">
            Cancel
          </a>
          <button
            type="submit"
