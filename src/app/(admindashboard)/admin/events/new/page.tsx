...
export default async function AdminCreateEventPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      {/* NOTE: file upload works with server action; don't set encType/method */}
      <form action={adminCreateEvent} className="space-y-6 rounded-xl border bg-white p-6 shadow-sm">
        <div className="grid gap-4 sm:grid-cols-2">
          <Field id="name" label="Event Name" required />
          <Field id="organization_name" label="Organization" />
          <div className="sm:col-span-2">
            <Label htmlFor="description">Description</Label>
            <textarea id="description" name="description" rows={4} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none" />
          </div>

          <Field id="event_date" label="Date" type="date" />
          <Field id="event_time" label="Time" type="time" />
          <Field id="location" label="Location" />
          <Field id="category" label="Categories (comma separated)" />
          ...
          <div className="sm:col-span-2 grid gap-3 sm:grid-cols-2">
            <div>
              ...
              <select id="registration_fee_type" name="registration_fee_type" className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none">
                <option value="free">Free</option>
                <option value="paid">Paid</option>
              </select>
            </div>
            <Field id="registration_fee_amount" label="Fee Amount (if paid)" type="number"  />
          </div>

          <Field id="member_limit" label="Max Attendees (leave blank = unlimited)" type="number"  />
          ...
          <div className="sm:col-span-2 grid gap-3 sm:grid-cols-2">
            <div>
              ...
              <select id="status" name="status" className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
...
function Field({ id, label, type="text", required=false, ...rest }: { id: string; label: string; type?: string; required?: boolean }) {
  return (
    <div>
      <Label htmlFor={id}>{label}</Label>
      <input id={id} name={id} type={type} required={required} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none" {...rest} />
    </div>
  );
}
